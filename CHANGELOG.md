...
- **File watcher spamming errors for subagent files** - Skip subagent `.jsonl` files in `/subagents/` directories. These files can't be imported independently.
...
- **File watcher not detecting new sessions on macOS** - Watch directories directly instead of using glob patterns. Chokidar glob patterns with absolute paths don't work reliably on macOS.
...
- **File watcher not detecting new sessions on macOS** - Fixed chokidar configuration to use absolute paths and polling on macOS for reliable FSEvents detection. New Claude and Gemini sessions are now detected immediately.
...
- **Browser opens before server ready** - Fixed race condition where browser would open before server finished initializing, causing ERR_CONNECTION_REFUSED. Now polls `/api/health` endpoint before opening browser.
...
- **SPA routing crash** - Fixed "NotFoundError: Not Found" crash when refreshing browser on routes like `/sessions`.
- **Multi-agent Quick Start** - Updated empty state to show `claude / codex / gemini`.
...
- **Session Player Toolbar** - Simplified toolbar by hiding Checkpoints and Summary buttons. Impact Analysis (Insights) button remains visible.

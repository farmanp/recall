...
export function HealthStatus() {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {data?.database && <span className="text-sm text-gray-500">DB: {data.database}</span>}
    </div>
  );
}
...
export function SessionList() {
  ...
  if (error) {
    return <div className="p-4 text-red-500">Error loading sessions: {error.message}</div>;
  }
    ...
    <div className="border rounded p-4">
      ...
      >
        <div className="flex-1">
          ...
          {session.user_prompt && (
            <p className="text-sm text-gray-600 line-clamp-2">{session.user_prompt}</p>
          )}
          <p className="text-xs text-gray-500 mt-2">
            Started: {new Date(session.started_at).toLocaleString()}
          </p>
        </div>
        <button className="text-gray-400 hover:text-gray-600">{expanded ? '▼' : '▶'}</button>
      </div>
...
function SessionEventsList({ sessionId }: { sessionId: number }) {
  ...
  if (error) {
    return <div className="mt-4 p-4 border rounded text-red-500">Error: {error.message}</div>;
  }
  ...
  return (
    <div className="mt-4 space-y-4">
      <div className="flex gap-2">
        ...
        <span className="px-3 py-2 text-sm text-gray-500">{data?.total} events</span>
      </div>
      ...
      >
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className={`px-2 py-0.5 rounded text-xs ${eventTypeColors[event.event_type]}`}>
              {event.event_type}
            </span>
            {event.obs_type && (
              <span className={`px-2 py-0.5 rounded text-xs ${obsTypeColors[event.obs_type]}`}>
                {event.obs_type}
              </span>
            )}
            {event.prompt_number !== null && (
              <span className="text-xs text-gray-500">Prompt #{event.prompt_number}</span>
            )}
          </div>
          {event.title && <h4 className="font-semibold text-sm mb-1">{event.title}</h4>}
          {event.subtitle && <p className="text-sm text-gray-600 mb-1">{event.subtitle}</p>}
          <p className="text-sm text-gray-700 line-clamp-2">{event.text}</p>
          <p className="text-xs text-gray-500 mt-1">{new Date(event.ts).toLocaleString()}</p>
        </div>
